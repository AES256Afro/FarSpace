...
import { bond, bondLabel, findStation, ledger, XP_STEPS } from "../world";
...
export class RosterScene implements Scene {
  ...
  draw(g: Game, ctx: CanvasRenderingContext2D): void {
    ...
    p.crew.forEach((c, i) => {
      ...
      drawText(ctx, `${c.name.toUpperCase()} - ${ROLE_INFO[c.role].label} ${"*".repeat(c.skill)}${c.sick ? " - LAID UP (" + c.sick.kind.toUpperCase() + ")" : ""}`, 12, y, sel ? PAL.white : PAL.ui);
