...
export class InteriorScene implements Scene {
  ...
  cardNight(g: Game): void {
    ...
    const enc: Encounter = { id: "cards", where: "space", title: "CARD NIGHT", weight: 0,
      text: `The galley table's been cleared and somebody's found a deck with the sixes marked. ${players.map((c) => c.name.split(" ")[0]).join(", ")} are already dealt in, and there's a chair with your name on it, or there isn't, depending on what you say next.${players.some((c) => (c.trait ?? "").includes("rock") || (c.trait ?? "").includes("spin")) ? " Somebody raises a cup before the first hand: 'To the churn.' Everybody drinks, even the ones who don't know what it means." : ""}`,
      options: [
  ...
  readWall(g: Game): void {
    ...
    if (p.flags?.shipCrew) { const wear = Math.round(p.wear ?? 0); const verdict = wear > 70 ? "YOU RUN ME HARD. I'D LIKE THAT NOTED, AND A YARD." : (p.rescues ?? 0) > (p.kills ?? 0) ? "MORE PEOPLE PULLED OUT THAN PUT DOWN. I'M PROUD OF THAT. I'M ALLOWED." : (p.kills ?? 0) >= 25 ? "YOU FIGHT WELL. I WISH YOU DIDN'T HAVE TO. I'M GLAD IT'S YOU." : "STEADY HANDS, MOST DAYS. THE OTHER DAYS I DON'T MENTION."; const extra = p.flags?.quietleg ? " YOU KEPT THE QUIET LEG. I HAVEN'T FORGOTTEN." : p.flags?.hundredth ? " A HUNDRED ON THE COUNT. I WAS THERE FOR ALL OF THEM." : (p.ribbons ?? 0) >= 3 ? " THREE RIBBONS. ALL CROOKED. I'D STRAIGHTEN THEM IF I HAD HANDS." : p.catchphrase ? ` AND '${p.catchphrase.toUpperCase()}' EVERY TIME. I'VE STOPPED MINDING. I MIND A LITTLE.` : ""; lines.push(`THE SHIP'S REVIEW OF THE CAPTAIN: "${verdict}${extra}"`); }
    if (p.flags?.shipCrew || p.voiceName) opts.push({ label: "THE GALLEY DOOR (SHIP'S NEWSLETTER)", hint: "The ship keeps a paper. Nobody asked it to.", result: (g2) => { const enc2: Encounter = { id: "newsletter", where: "space", title: "THE GALLEY DOOR", weight: 0, text: shipNewsletter(g2.world).join("\n"), options: [{ label: "CLOSE", result: () => "" }] }; (g2.scenes["encounter"] as EncounterScene).open(g2, enc2, "interior", true); flag(g2, "newsletter"); return ""; } });
    opts.push({ label: p.catchphrase ? "CHANGE THE WORD YOU GIVE ON UNDOCK" : "CHOOSE THE WORD YOU GIVE ON UNDOCK", hint: "Every captain has one. The helm answers it.", result: (g2) => { const t = (ask("The word you give when the clamp lets go. Short.", p.catchphrase ?? "") ?? "").trim().slice(0, 32); if (!t) { delete p.catchphrase; return "NO WORD. THE CLAMP LETS GO IN SILENCE, WHICH IS ALSO A STYLE."; } p.catchphrase = t; flag(g2, "theword"); return `'${t.toUpperCase()}.' THE HELM TRIES IT BACK UNDER THEIR BREATH. IT'LL DO.`; } });
