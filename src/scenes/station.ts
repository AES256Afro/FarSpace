...
export class StationScene implements Scene {
  ...
  enter(g: Game): void {
    ...
    { const rk = commandRank(p); const who = `${p.shipName ? p.shipName + ", " : ""}${registry(g.world)}, ${rk !== "SKIPPER" ? rk + ", " : ""}${title ? title + ", " : ""}`; g.toast(stationHour(this.station).night ? `${this.station.name.toUpperCase()} NIGHT WATCH: ${who}BAY ${bay}. KEEP IT QUIET, THE DAY SHIFT IS ASLEEP` : `${this.station.name.toUpperCase()} CONTROL: ${who}CLEARANCE GRANTED, BAY ${bay}`); }
