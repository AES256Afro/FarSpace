...
export class StationScene implements Scene {
  ...
  completeMissionInner(g: Game, m: Mission): void {
    ...
    if (m.kind === "post") { const note = postDelivered(g.world, new RNG((g.world.seed ^ Math.floor(g.world.time * 13)) >>> 0)); if (note) g.toast(note); if ((p.postRuns ?? 0) >= 10) flag(g, "postman"); }
    if (m.syndicate) {
