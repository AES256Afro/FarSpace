...
export class TitleScene implements Scene {
  ...
  options(g: Game): { label: string; sub: string; act: () => void }[] {
    ...
    opts.push({ label: "CONTROLS", sub: "Every key, by where you are", act: () => { g.settingsReturn = "title"; g.setScene("help"); } });
    opts.push({ label: "HANDBOOK", sub: "Every system in the game, a few lines each", act: () => { g.settingsReturn = "title"; g.setScene("almanac"); } });
    ...
    { const ver = typeof __APP_VERSION__ === "string" ? __APP_VERSION__ : "?"; const unread = settings().whatsNewSeen !== ver; opts.push({ label: unread ? "WHAT'S NEW *" : "WHAT'S NEW", sub: unread ? "New since you last looked" : "Changes since you last flew", act: () => g.setScene("whatsnew") }); }
