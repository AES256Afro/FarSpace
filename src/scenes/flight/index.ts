...
export class FlightScene implements Scene {
  ...
  pauseOptions(g: Game): { label: string; act: () => void }[] {
    return [
      ...
      ...(firstOfficer(g.world.player) && !g.world.player.numberOneLeg && !this.docking ? [{ label: "NUMBER ONE HAS THE SHIP THIS LEG", act: () => { this.paused = false; const p2 = g.world.player; const fo2 = firstOfficer(p2)!; p2.numberOneLeg = true; this.autopilot = true; fo2.loyalty = (fo2.loyalty ?? 0) + 0.3; for (const c of p2.crew) c.morale = Math.min(100, c.morale + 2); logEntry(g.world, `Gave ${fo2.name} the ship for the leg`); this.comms.push({ from: fo2.name.split(" ")[0].toUpperCase(), text: "I HAVE THE SHIP. GO AND SLEEP, CAPTAIN. THAT'S AN ORDER, WHICH I CAN'T GIVE, SO IT'S A REQUEST.", life: 8, color: PAL.grey }); flag(g, "numberoneleg"); } }] : []),
  ...
  dockAt(g: Game, st: StationDef): boolean {
    ...
    g.lastBay = bay;
    this.cruise = false; this.autopilot = false;
    if (!hold && this.comms.length < 4) { const rk = commandRank(p); const regular = dockingsAt(p, st.id) >= 3; const who = rk !== "SKIPPER" ? `${rk} ${(p.shipName ?? "VESSEL").toUpperCase()}` : (p.shipName ?? "VESSEL").toUpperCase(); this.comms.push({ from: `${st.name.toUpperCase()} CONTROL`, text: `${who}, CLEARED BAY ${bay}. ${regular ? "WELCOME BACK. YOUR USUAL." : `WELCOME TO ${st.name.toUpperCase()}.`}${p.flags?.freeman && isBeltStation(st) ? " THE ROCK SAYS HELLO." : ""}`, life: 7, color: PAL.greyDark }); }
  ...
  updateAmbient(g: Game, dt: number): void {
    ...
    { const fo0 = firstOfficer(p); if (fo0 && !fo0.sick && p.leg && g.world.time - p.leg.t0 > 8 * 3600 && !(p.flags ?? {}).longLegNudged && this.comms.length < 4) { (p.flags ??= {}).longLegNudged = true; this.comms.push({ from: fo0.name.split(" ")[0].toUpperCase(), text: "EIGHT HOURS SINCE THE CLAMP, CAPTAIN. THE CREW ARE COUNTING RIVETS. I'D LIKE A PORT ON THE CHART, ANY PORT, BEFORE THEY START NAMING THEM.", life: 9, color: PAL.warn }); } }
  ...
  updateEncounters(g: Game, dt: number): void {
    ...
    if (!enc) return;
    sfx.thrust(false); noteLeg(p, "cards", g.world.time);
  ...
  doJump(g: Game, targetId: string, guarded: boolean): void {
    ...
    p.fuel -= cost; this.hardBurn = false; noteLeg(p, "jumps", g.world.time);
