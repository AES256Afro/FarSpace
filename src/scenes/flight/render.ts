...
export function drawFlight(fs: FlightScene, g: Game, ctx: CanvasRenderingContext2D): void {
  ...
  for (const wd of wondersIn(g.world, sys.id)) {
    ...
    else if (wd.kind === "lantern") { for (let i = 0; i < 5; i++) { ctx.fillStyle = i % 2 ? "#ff9a3a" : "#ffb347"; ctx.globalAlpha = 0.18 + 0.1 * Math.sin(t * 0.7 + i) + (dist(p.x, p.y, wd.x, wd.y) < 600 ? 0.15 : 0); ctx.beginPath(); ctx.arc(sx + Math.cos(i * 1.3) * 40 * z, sy + Math.sin(i * 1.9) * 30 * z, (90 + i * 22) * z, 0, Math.PI * 2); ctx.fill(); } ctx.globalAlpha = 1; }
