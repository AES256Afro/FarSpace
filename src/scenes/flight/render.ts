...
export function drawHud(fs: FlightScene, g: Game, ctx: CanvasRenderingContext2D): void {
  ...
  { const fo = firstOfficer(p); const gun = p.crew.find((c) => c.role === "gunner" && !c.sick); const eng = p.crew.find((c) => c.role === "engineer" && !c.sick); const parts: string[] = []; if (fs.autopilot && fo) parts.push(`CONN: ${fo.name.split(" ")[0].toUpperCase()}`); if (fs.alert === 2 && gun) parts.push(`TACTICAL: ${gun.name.split(" ")[0].toUpperCase()}`); if (fs.alert === 2 && eng && p.systems.some((s) => s.health < 100)) parts.push(`DAMAGE CONTROL: ${eng.name.split(" ")[0].toUpperCase()}`); if (parts.length) { const t = parts.join("  "); drawText(ctx, t, VW - textWidth(t) - 4, VH - 34, PAL.greyDark); } }
