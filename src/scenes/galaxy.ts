...
export class GalaxyScene implements Scene {
  ...
  draw(g: Game, ctx: CanvasRenderingContext2D): void {
    ...
    for (const sys of Object.values(w.systems)) {
      ...
      { const ev = galaxyEventAt(w, sys.id); if (ev) { ctx.fillStyle = ev.kind === "comet" ? PAL.mining : ev.kind === "flare" ? PAL.warn : ev.kind === "festival" ? PAL.gold : PAL.grey; ctx.fillRect(Math.round(x) - 7, Math.round(y) + 4, 2, 2); } }
    ...
    if (this.selected) {
      ...
      { const ev = galaxyEventAt(w, sys.id); if (ev) { drawText(ctx, ev.kind === "comet" ? "COMET: RICH BELT" : ev.kind === "flare" ? "SOLAR FLARE: HOT, SHORT SCANS" : ev.kind === "festival" ? "FESTIVAL WEEK" : "DOCK STRIKE", px + 6, y, ev.kind === "flare" || ev.kind === "strike" ? PAL.warn : PAL.gold); y += 9; } }
