...
export class GalaxyScene implements Scene {
  ...
  draw(g: Game, ctx: CanvasRenderingContext2D): void {
    ...
    for (const sys of Object.values(w.systems)) {
      ...
      { const ev = galaxyEventAt(w, sys.id); if (ev) { ctx.fillStyle = ev.kind === "comet" ? PAL.mining : ev.kind === "flare" || ev.kind === "storm" ? PAL.warn : ev.kind === "festival" ? PAL.gold : PAL.grey; ctx.fillRect(Math.round(x) - 7, Math.round(y) + 4, 2, 2); } }
