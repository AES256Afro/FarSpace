...
export class VistaScene implements Scene {
  ...
  draw(g: Game, ctx: CanvasRenderingContext2D): void {
    ...
    for (const wd of wondersIn(w, sys.id)) place(wd.x, wd.y, wd.id, (sx, sy, app) => { ctx.globalAlpha = 0.35; ctx.fillStyle = wd.kind === "pulsar" || wd.kind === "glass" ? "#8ec9f0" : wd.kind === "lantern" || wd.kind === "twins" ? "#ffb347" : "#9aa5bd"; ctx.beginPath(); ctx.arc(sx, sy, app, 0, Math.PI * 2); ctx.fill(); ctx.globalAlpha = 1; ctx.fillStyle = "#f2f4ff"; ctx.fillRect(sx - 1, sy - 1, 2, 2); }, 60, 900, 80, wd.seen ? wd.name.toUpperCase() : "SOMETHING VAST", wd.desc.toUpperCase());
