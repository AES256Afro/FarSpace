...
import { generateShipDeck, type ShipDeck, type RepairProgress } from "../core/shipdeck";
...
export class RepairScene implements Scene {
  ...
  enter(g: Game): void {
    ...
    if (!g.repairTarget && !this.tender) { (g.scenes.flight as FlightScene).resumeNext = true; g.setScene("flight"); return; }
    const target = this.tender ? g.tenderMission! : g.repairTarget!;
    const identity = this.tender ? g.tenderMission!.id : `${g.world.player.systemId}:${g.repairTarget!.name ?? "ship"}:${Math.round(g.repairTarget!.x)}:${Math.round(g.repairTarget!.y)}`;
    const seed = hashStr(`${g.world.seed}:${identity}`);
    this.progress = target.repairInterior ??= { seed, health: {}, fires: [] };
    this.layout = generateShipDeck(this.progress.seed);
    this.deck = this.layout.tiles;
    this.rng = new RNG(this.progress.seed ^ 0x51a7);
  ...
  solid(tx: number, ty: number): boolean {
    ...
    return this.layout.systems.some((s) => s.tx === tx && s.ty === ty);
  }
  done(): boolean { return this.layout.systems.every((s) => this.health[s.ch] >= 100); }
  ...
  leave(g: Game, success: boolean): void {
    if (this.tender) {
      ...
      else g.toast("REPAIR PROGRESS KEPT. THE TENDER STAYS OPEN.");
  ...
  update(g: Game, dt: number): void {
    ...
    if (this.health.R < 60 && Math.random() < dt * 0.06 && this.fires.length < 4) {
      const reactor = this.layout.systems.find(s => s.ch === "R")!;
      const f = { tx: reactor.tx + this.rng.int(-1, 1), ty: reactor.ty + this.rng.int(-1, 1) };
    ...
    const sys = this.layout.systems.find((s) => Math.hypot(s.tx * T + T / 2 - this.px, s.ty * T + T / 2 - this.py) < 18);
  ...
  draw(g: Game, ctx: CanvasRenderingContext2D): void {
    ...
    for (const s of this.layout.systems) {
    ...
    drawText(ctx, this.tender ? "STATION PLANT - ENGINEERING TENDER" : `${this.layout.name} / REPAIR TEAM`, 8, 6, PAL.white);
    drawText(ctx, `SUIT O2 ${Math.round(this.o2)}%   PARTS ${g.world.player.cargo.parts ?? 0}   ${this.layout.systems.map((s) => `${s.ch} ${Math.round(this.health[s.ch])}%`).join("  ")}`, 8, 15, this.o2 < 30 ? PAL.danger : PAL.grey);
    drawText(ctx, `${this.layout.rooms.length} COMPARTMENTS / WASD MOVE / HOLD E REPAIR`, 8, 24, PAL.greyDark);
