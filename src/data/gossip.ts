...
export function concourseGossip(w: World, st: StationDef, rng: RNG): string[] {
  ...
  if (isBeltStation(st) && !st.military) pool.push("'THE COUNCIL ROOM IS BETWEEN THE MARKET AND THE HARBOUR OFFICE. VISITORS CAN LISTEN. BRING YOUR OWN OPINION.'");
  if (p.council?.stationId === st.id) pool.push("'YOUR CHAIR'S STILL THERE. THEY HAD TO WRITE YOUR NAME ON IT. PEOPLE KEPT BORROWING IT FOR THE KETTLE.'");
  ...
  if (st.type === "mining" || st.type === "refinery") pool.push("'INNERS PAY LATE AND TIP EARLY. BELT CREWS THE OTHER WAY ROUND. LEARN WHICH ONE YOU ARE.'", "'AIR, WATER, AND THE RECYCLERS. THAT'S THE WHOLE ECONOMY OUT HERE. THE REST IS DECORATION.'");
  if (p.flags?.freeman) pool.push("'THAT'S A FREEMAN OF THE BELT. THREE ROCKS OWE THEM. THE YARD KNOWS. THE BAR DEFINITELY KNOWS.'");
  if (p.flags?.belt) pool.push("'A ROCK HOPPER CAME IN ON HALF A TANK OF AIR. SOMEBODY OUT THERE SHARED. THE BELT REMEMBERS THAT.'", "'THAT'S THE CAPTAIN WHO SHARED AIR. YARD'S TEN UNDER FOR THEM ON ANY ROCK. DON'T TELL THE INNERS.'");
  if (p.flags?.singersGuided) pool.push("'THE SINGING HULLS FOLLOWED A CAPTAIN TO A BEACON LAST WEEK. FOLLOWED. LIKE DUCKLINGS. THE SCIENCE POSTS ARE BESIDE THEMSELVES.'");
  if (p.flags?.firstContact) pool.push("'THEY SAY A CAPTAIN TALKED TO ONE OF THE SINGING HULLS. WITH A TRANSLATOR. OR A SONG. DEPENDS WHO'S TELLING IT.'");
