...
export function crewChatter(w: World, a: CrewMember, b: CrewMember, rng: RNG): string {
  ...
  if ((p.service?.completed ?? 0) > 0) pool.push("THE DUTY CLERK READ OUR WHOLE REPORT. I WATCHED. I THINK THAT IS WHY THE CAPTAIN LIKES THAT OFFICE.", "WE HAVE A SERVICE GRADE NOW. THE KETTLE REMAINS AN INDEPENDENT CONTRACTOR.");
  if ((p.shipSim?.runs ?? 0) > 0) pool.push("THE SHIP'S PROGRAM GOT THE CAPTAIN RIGHT. IT GOT MY HAIR WRONG. I HAVE SUBMITTED A CORRECTION.", "IT SAYS THE BRIDGE WAS TO SCALE. I HAVE A TAPE MEASURE. WE WILL SETTLE THIS AFTER WATCH.");
...
const PAX_TO_ROLE: Record<string, Record<string, string[]>> = {
  ...
  prisoner: {
    pilot: ["YOU FLY LIKE SOMEBODY WHO'S NEVER BEEN CHASED. I MEAN THAT KINDLY."],
    engineer: ["THESE IRONS ARE A GUILD PATTERN. THE LOCK'S ON THE LEFT. I'M JUST SAYING I KNOW."],
    medic: ["I DIDN'T DO IT. ... ALL RIGHT, I DID IT. BUT NOT THE WAY THEY SAY."],
    gunner: ["YOU DON'T HAVE TO STAND THERE THE WHOLE TIME. ... YOU'RE GOING TO STAND THERE THE WHOLE TIME."],
  },
  patient: {
