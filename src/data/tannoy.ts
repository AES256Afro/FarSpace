...
export function tannoyLines(w: World, st: StationDef, rng: RNG, now = Date.now()): string[] {
  ...
  if (p.flags?.inspection && p.dockedAt === st.id) pool.push("THE SHIP THAT PASSED INSPECTION IS IN. THE YARD IS SHOWING IT TO THE APPRENTICES. TRY NOT TO SCRATCH IT.");
  ...
  if (st.type === "research" && p.singersHome && !p.flags?.singersHomeDone) pool.push("THE SURVEY HAS A CHART ON FILE FROM A HULL AT THE CLAMP. FOLLOW THE LIGHT IS ON THE BOARD. THE SURVEY IS TRYING NOT TO LOOK EXCITED. THE SURVEY IS FAILING.");
  if (st.type === "research" && p.flags?.singershome) pool.push("THE SURVEY THANKS THE HULL THAT FOLLOWED THE LIGHT. THE SURVEY HAS THE RECORDING. THE SURVEY HAS PLAYED IT NINE TIMES TODAY. THE SURVEY WILL STOP AT TEN.");
  if (isBeltStation(st) && p.raceBest?.[st.id] !== undefined) pool.push(`THE ROCK'S RING BOARD: ${(p.shipName ?? "THE VISITING HULL").toUpperCase()} AT ${p.raceBest[st.id].toFixed(1)} SECONDS. THE KIDS HAVE CHALKED IT ON THE CLAMP WALL. AGAIN.`);
  ...
  if (isBeltStation(st) && p.flags?.rockkid) pool.push("WILL THE KIDS PLEASE STOP DRAWING ON THE CLAMP WALL. YES, IT'S A GOOD SHIP. YES, THE DOG IS FUNNY. STOP.");
  if (ev?.kind === "drought" && ev.stationId === st.id) pool.push("WATER RATION IS TWO LITRES A HEAD UNTIL A TANK COMES IN. SHOWERS ARE A RUMOUR.", "ANY HULL WITH WATER IN THE HOLD: THE MARKET WILL TAKE ALL OF IT, AND THE ROCK WILL REMEMBER YOUR NAME.", "THE ICE LINE CREW ARE OUT ON THE RIG. DON'T PAGE THEM. THEY KNOW.");
