...
export const ENCOUNTERS: Encounter[] = [
  ...
  },
  {
    ...
    options: [
      { label: "LEAD THEM TO THE LIGHT", hint: "A lit beacon or a wonder in this system", requires: (g) => infraAt(g.world, sys(g).id).some((i) => i.kind === "beacon" && infraLit(i)) || wondersIn(g.world, sys(g).id).length > 0, result: (g) => { { const wl = learnWord(p(g), "the light (sung falling, twice)"); if (wl && typeof g.toast === "function") g.toast(wl); } (p(g).flags ??= {}).singersGuided = true; (p(g).codex ??= {})["contact:THE SINGERS"] = 2; p(g).expData = (p(g).expData ?? 0) + 60; logEntry(g.world, "Led the singing hull to the brightest thing in the system"); return "YOU FLY SLOW AND THEY FOLLOW, SINGING THE RISING PHRASE. AT THE LIGHT THEY GO QUIET FOR A LONG MINUTE, THEN SING SOMETHING NEW. +60 DATA. THE CORE FILES IT UNDER 'THANKS'."; } },
  ...
  },
  {
    ...
    options: [
      { label: "SIGN THEM ON. CADET RATE", hint: "A berth; a rock-born cadet with no dockings; the belt hears", result: (g, rng) => { const c = genCrewCandidate(rng); c.docks = 0; c.cadet = true; c.trait = "born on a rock"; c.wage = Math.max(10, Math.round(c.wage * 0.6)); c.morale = 95; c.loyalty = 1.5; c.home = sys(g).stations.find((st) => isBeltStation(st))?.id; p(g).crew.push(c); (p(g).flags ??= {}).rockcadet = true; p(g).beltStanding = (p(g).beltStanding ?? 0) + 2; logEntry(g.world, `Signed on ${c.name}, the rock kid with the drawing, as a cadet`); return `THEY COME ACROSS WITH ONE BAG AND THE DRAWING, FOLDED SMALL, AND PIN IT UP IN THE BUNK ROOM BEFORE THEY'VE PUT THE BAG DOWN. ${c.name.toUpperCase()}, ${c.role.toUpperCase()}, CADET RATE, BORN ON A ROCK. THE BELT HEARS INSIDE THE HOUR.`; } },
  ...
  },
  {
    ...
    options: [
      { label: "SIGN THEM ON AS A CADET", hint: "A berth, a wage, no dockings; the crew will send them for a spanner", requires: (g) => berthsUsedW(p(g)) < hull(p(g).hullId).crewSlots, result: (g, rng) => { const c = genCrewCandidate(rng); c.docks = 0; c.cadet = true; c.skill = Math.max(1, c.skill - 1); c.wage = Math.max(10, Math.round(c.wage * 0.6)); c.morale = 90; c.loyalty = 1; p(g).crew.push(c); (p(g).flags ??= {}).cadet = true; logEntry(g.world, `Signed a stowaway on as a cadet: ${c.name}, ${c.role}`); return `THEY PICK A NAME THAT MIGHT BE THEIRS, ${c.name.toUpperCase()}, AND A JOB THEY SAY THEY CAN DO, WHICH IS ${c.role.toUpperCase()}, AND YOU BELIEVE ABOUT HALF OF IT. A BERTH, A WAGE AT CADET RATE, AND THE CREW ALREADY PLANNING SOMETHING WITH A SPANNER.`; } },
  ...
  },
  {
    ...
    options: [
      { label: "OF COURSE", result: (g) => { { (p(g).keepsakes ??= []).push("the ring from the galley wedding, borrowed and never returned"); if (p(g).keepsakes!.length > 8) p(g).keepsakes!.shift(); if ((p(g).flags?.shipCrew || p(g).voiceName) && typeof g.toast === "function") g.toast(`${(p(g).voiceName ?? p(g).shipName ?? "THE SHIP").toUpperCase()}: I DIMMED THE GALLEY LIGHTS FOR IT. NOBODY ASKED. I'M THE VENUE. I HAVE STANDARDS.`); } const cr = p(g).crew; const a = cr.find((x) => cr.some((y) => x !== y && bond(x, y) >= 3)); const b = a ? cr.find((y) => y !== a && bond(a, y) >= 3) : undefined; if (!a || !b) return "THE MOMENT PASSES."; for (const c of cr) c.morale = Math.min(100, c.morale + 10); a.loyalty = (a.loyalty ?? 0) + 1; b.loyalty = (b.loyalty ?? 0) + 1; (p(g).flags ??= {}).crewWed = true; logEntry(g.world, `Married ${a.name} and ${b.name} in the galley`); return `THE GALLEY, A TABLECLOTH, THE CAT ON THE TABLE. ${a.name.toUpperCase()} AND ${b.name.toUpperCase()}. THE WHOLE SHIP IS LATE FOR ITS WATCH AND NOBODY MINDS.`; } },
