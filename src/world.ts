...
export function chronicleText(w: World, callsign: string | null): string {
  ...
  { const contacts = Object.entries(p.codex ?? {}).filter(([k]) => k.startsWith("contact:")).map(([k, v]) => `${k.slice(8).toLowerCase()} (${v})`); if (contacts.length) { lines.push(""); lines.push("Contacts:"); lines.push(`  ${contacts.join("; ")}${p.singersHome ? `. The singers' chart points at ${w.systems[p.singersHome]?.name ?? "somewhere"}${p.flags?.singershome ? ", and we went" : ""}.` : "."}`); } }
