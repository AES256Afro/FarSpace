...
export function officeWrites(w: World, what: string): void {
  ...
  if (n === 6) { (w.mailQueue ??= []).push({ dueT: w.time + 900, from: "the office of anomalous incidents, formerly", text: "The representative has retired. There is no replacement; the budget meeting was decisive. Enclosed: the files. All of them. Every form you didn't return and the drawing. The office has decided that the correct custodian of the files is the hull that generated them. The office no longer exists, and would like that noted somewhere it can't be. Here, then.", gift: { data: 60 } }); (p.keepsakes ??= []).push("the office's files, all of them, in a box that hums"); if (p.keepsakes.length > 8) p.keepsakes.shift(); (p.flags ??= {}).officeClosed = true; }
...
export function chronicleText(w: World, callsign: string | null): string {
  ...
  { const fo = firstOfficer(p); if (fo) { const kills = p.kills ?? 0, rescues = p.rescues ?? 0; const mood = Math.round(p.crew.reduce((a, c) => a + c.morale, 0) / Math.max(1, p.crew.length));
    ...
    const more = `${fo.wasCadet ? " I came aboard for a spanner that doesn't exist. I'd like that in the record before somebody else puts it there." : ""}${p.flags?.stayed ? " The service offered me a chair. The captain asked me to stay, plainly. I'm still here; draw your own conclusion." : ""}${p.flags?.secondsbar ? " I've drunk at the seconds' bar. Every Number One has the same story about their captain. Mine is better." : ""}${p.flags?.numberoneleg ? " They've given me the ship for a leg, which is trust, or tiredness, and I'll take either." : ""}${p.flags?.sameleave ? " They let two of ours take the same leave without a speech. Crews notice what a captain doesn't say." : ""}${p.crew.some((c) => (c.docks ?? 0) === 0) ? " There's a cadet aboard. I've told them the captain doesn't bite. I've told the captain the same." : ""}`;
  ...
  { const bits: string[] = []; bits.push(`${commandRank(p).toLowerCase()} by deeds (${(p.achievements ?? []).length})`); if ((p.beltStanding ?? 0) > 0) bits.push(`belt standing ${Math.min(BELT_FREEMAN_AT, p.beltStanding ?? 0)} of ${BELT_FREEMAN_AT}${p.flags?.freeman ? ", a freeman" : p.flags?.freemanLost ? ", the name taken back" : ""}`); if ((p.ribbons ?? 0) > 0) bits.push(`${p.ribbons} ribbon${p.ribbons === 1 ? "" : "s"}`); if ((p.inquiries ?? 0) > 0) bits.push(`${p.inquiries} board${p.inquiries === 1 ? "" : "s"} of inquiry`); if ((p.hearings ?? 0) > 0) bits.push(`${p.hearings} hearing${p.hearings === 1 ? "" : "s"} over the rule`); if ((p.prisoners ?? 0) > 0) bits.push(`${p.prisoners} prisoner${p.prisoners === 1 ? "" : "s"} delivered`); if ((p.evacuated ?? 0) > 0) bits.push(`${p.evacuated} carried out of bad weeks`); if ((p.awayTrips ?? 0) > 0) bits.push(`${p.awayTrips} landing${p.awayTrips === 1 ? "" : "s"} with someone in the seat beside`); if (p.flags?.freemanTwice) bits.push("the freeman's name earned twice"); lines.push(""); lines.push("Standing:"); lines.push(`  ${bits.join("; ")}.`); }
  { const contacts = Object.entries(p.codex ?? {}).filter(([k]) => k.startsWith("contact:")).map(([k, v]) => `${k.slice(8).toLowerCase()} (${v}${k.includes("OFFICE") && p.flags?.officeClosed ? ", closed; the files are aboard" : ""})`); if (contacts.length) { lines.push(""); lines.push("Contacts:"); lines.push(`  ${contacts.join("; ")}${p.singersHome ? `. The singers' chart points at ${w.systems[p.singersHome]?.name ?? "somewhere"}${p.flags?.singershome ? ", and we went" : ""}.` : "."}`); } }
...
export function shipNewsletter(w: World): string[] {
  ...
  { const cmd = (p.alumni ?? []).find((a) => a.command); if (cmd) out.push(`${cmd.name.split(" ")[0].toUpperCase()} HAS THEIR OWN SHIP. THE EDITOR IS PROUD. THE EDITOR IS ALSO SHORT A NUMBER ONE AND WOULD LIKE THAT NOTED.`); }
  if (p.flags?.singershome) out.push("WE WENT WHERE THE SINGERS LIVE. THE EDITOR WAS SUNG TO. THE EDITOR WILL NOT BE TAKING QUESTIONS.");
...
export function beltGain(w: World, n: number): string | null {
  ...
  if (before < BELT_FREEMAN_AT && p.beltStanding >= BELT_FREEMAN_AT && p.flags.freemanLost) { p.flags.freeman = true; delete p.flags.freemanLost; (p.flags ??= {}).freemanTwice = true; logEntry(w, "The belt gave the freeman's name back, the second time"); return "THREE ROCKS SAY THE NAME'S YOURS AGAIN. TWICE IS RARER THAN ONCE. THE BELT KNOWS THAT TOO."; }
