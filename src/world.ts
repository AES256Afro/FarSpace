...
export function chronicleText(w: World, callsign: string | null): string {
  ...
  { const bits: string[] = []; bits.push(`${commandRank(p).toLowerCase()} by deeds (${(p.achievements ?? []).length})`); if ((p.beltStanding ?? 0) > 0) bits.push(`belt standing ${Math.min(BELT_FREEMAN_AT, p.beltStanding ?? 0)} of ${BELT_FREEMAN_AT}${p.flags?.freeman ? ", a freeman" : p.flags?.freemanLost ? ", the name taken back" : ""}`); if ((p.ribbons ?? 0) > 0) bits.push(`${p.ribbons} ribbon${p.ribbons === 1 ? "" : "s"}`); if ((p.inquiries ?? 0) > 0) bits.push(`${p.inquiries} board${p.inquiries === 1 ? "" : "s"} of inquiry`); if ((p.hearings ?? 0) > 0) bits.push(`${p.hearings} hearing${p.hearings === 1 ? "" : "s"} over the rule`); if ((p.prisoners ?? 0) > 0) bits.push(`${p.prisoners} prisoner${p.prisoners === 1 ? "" : "s"} delivered`); if ((p.evacuated ?? 0) > 0) bits.push(`${p.evacuated} carried out of bad weeks`); lines.push(""); lines.push("Standing:"); lines.push(`  ${bits.join("; ")}.`); }
