...
// whole thing can be dismissed with T. Existing saves start with it off.
...
export const STEPS = [
  ...
  { text: "BOARD YOUR SHIP (I) AND LOOK AROUND", reward: 300 },
];
...
export function tutorialUpdate(g: Game): void {
  ...
  if (g.input.wasPressed("t")) { p.tutorial = -1; g.toast("FLIGHT SCHOOL DISMISSED"); return; }
...
export function drawTutorial(g: Game, ctx: CanvasRenderingContext2D, y = 72): void {
  ...
  drawText(ctx, "[T] SKIP", x + w - textWidth("[T] SKIP") - 6, y, PAL.greyDark);
