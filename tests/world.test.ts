...
describe("station hours and the tannoy", () => {
  ...
  it("an inspection scores wear, morale, the deck and the hull", () => {
    const w = generateWorld(61, { realGalaxy: true }); const p = w.player;
    p.wear = 10; p.crew = [genCrewCandidate(new RNG(1))]; p.crew[0].morale = 80; p.furnishings = ["plant", "rug"]; p.hull = p.hullMax;
    expect(inspectionScore(p)).toEqual({ score: 4, notes: [] });
    p.wear = 80; p.hull = 1; expect(inspectionScore(p).score).toBe(2); expect(inspectionScore(p).notes).toEqual(["THE FRAME RATTLES", "THE HULL IS PATCHED"]);
    const st = Object.values(w.systems).flatMap((s) => s.stations).find((s) => s.factionId !== "vex")!; p.rep[st.factionId] = 40;
    expect(inspectionDue(w, st)).toBe(false); p.achievements = Array.from({ length: 70 }, (_, i) => `a${i}`);
    expect(typeof inspectionDue(w, st)).toBe("boolean");
    expect(chronicleText(w, null)).not.toContain("Captain's log, last entries:"); logEntry(w, "A test entry"); expect(chronicleText(w, null)).toContain("Captain's log, last entries:");
  });
  ...
  it("a belt rock goes independent for a week: the register opens and the market moves", () => {
    ...
    let sec: GalaxyEvent | null = null;
    for (let i = 0; i < 400 && !sec; i++) { w.galaxyEvent = null; tickGalaxyEvents(w, new RNG(i)); if (w.galaxyEvent?.kind === "secession") sec = w.galaxyEvent; }
    expect(sec).not.toBeNull();
    const st = findStation(w, sec!.stationId!)!.st; expect(isBeltStation(st)).toBe(true);
    expect(secessionAt(w, st.id)).not.toBeNull(); expect(tannoyLines(w, st, new RNG(1)).some((l) => l.includes("INDEPENDENT UNTIL MONDAY"))).toBe(true);
    w.time = sec!.until + 1; expect(secessionAt(w, st.id)).toBeNull();
  });
  it("parley odds climb with a gunner, a rank and a reputation", () => {
    const w = generateWorld(59, { realGalaxy: true }); const p = w.player; p.crew = []; p.kills = 0; p.achievements = [];
    expect(parleyChance(p)).toBeCloseTo(0.35, 5);
    p.crew = [{ ...genCrewCandidate(new RNG(1)), role: "gunner", sick: null } as any]; expect(parleyChance(p)).toBeCloseTo(0.55, 5);
    p.achievements = Array.from({ length: 30 }, (_, i) => `a${i}`); expect(parleyChance(p)).toBeCloseTo(0.7, 5);
    p.kills = 30; expect(parleyChance(p)).toBeCloseTo(0.9, 5);
  });
  ...
  it("the crew want a word when morale sinks, once a week", () => {
    ...
    p.crew = [genCrewCandidate(new RNG(1)), genCrewCandidate(new RNG(2))]; for (const c of p.crew) c.morale = 60;
    expect(grievanceDue(w)).toBe(false); for (const c of p.crew) c.morale = 20; expect(grievanceDue(w)).toBe(true);
    grievanceHeard(w); expect(grievanceDue(w)).toBe(false);
  });
  ...
  it("the belt remembers: mining and refinery yards give the sharer their rate", () => {
    ...
    expect(beltGain(w, 1)).toBeNull(); expect(beltGain(w, 1)).toBeNull(); expect(beltGain(w, 1)).toContain("FREEMAN"); expect(beltRate(p, belt)).toBe(0.85); expect(p.flags?.freeman).toBe(true);
  ...
  it("the senior staff report by department and a focus changes wear, fuel and the sick", () => {
    ...
    expect(rep.length).toBe(5); expect(rep[0]).toContain("(ENGINES)");
    p.numberOne = p.crew[0].name; const rep1 = briefingReports(w); expect(rep1.length).toBe(6); expect(rep1.find((l) => l.includes("(NUMBER ONE)"))).toBeTruthy(); delete p.numberOne; expect(rep[0]).toContain("WEAR AT 65%"); expect(rep[1]).toContain("(SICKBAY)"); expect(rep[2]).toContain("NO PILOT"); expect(rep[3]).toContain("(THE SHIP)"); expect(rep[4]).toContain("NO GUNNER");
